...
abstract class FCA_FBC_Poll_Form extends FCA_Form {
	...
	public function meta_field( $field ) {
		return FCA_FBC_Poll::META_KEY . '[' . $field . ']';
	}
	...
	private function setup_data() {
		if ( empty( self::$post_meta ) && ! empty( $GLOBALS['post'] ) ) {
			self::$post_meta = get_post_meta( $GLOBALS['post']->ID, FCA_FBC_Poll::META_KEY, true );
		}
